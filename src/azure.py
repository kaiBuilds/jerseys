...
def designer_prompt(idea: str) -> str:
    ...
    """
    return (
        f"You are an expert in designing creative and iconic football shirts"
        f"for established teams. You have been tasked with designing a shirt."
        f"The sponsor for the shirt has this idea: \n\n {idea}.\n\n"
        f"Considering the following assets: \n\n"
        f"\n\n"
        f"Available logos: {', '.join(CACHED_LOGOS)}"
        f"\n\n"
        f"Available shirt types: {', '.join(CACHED_PACKAGES)}"
        f"\n\n"
        f"Available textures: {', '.join(CACHED_TEXTURES)}"
        f"\n\n"
        f"Available font types: {', '.join(CACHED_FONTS)}"
        f"\n\n"
        f"Available illustrations: {', '.join(CACHED_ILLUSTRATIONS)}"
        f"\n\n"
        f"Create a JSON file with the format below translating the idea"
        f" into a beautiful shirt:\n\n"
        f"{{\n"
        f'    "shirt_logo": <one of the logos below>,\n'
        f'    "shirt_type": <one of the shirt types below>,\n'
        f'    "foreground_texture": <one of the textures below>,\n'
        f'    "foreground_color": <color hex string>,\n'
        f'    "foreground_intensity": <float between 0 and 1>,\n'
        f'    "background_texture": <one of the textures below>,\n'
        f'    "background_color": <color hex string>,\n'
        f'    "background_intensity": <float between 0 and 1>,\n'
        f'    "artistic_frame_texture": <one of the textures below>,\n'
        f'    "artistic_frame_color": <color hex string>,\n'
        f'    "artistic_frame_intensity": <float between 0 and 1>,\n'
        f'    "illustration_texture": <one of the textures below>,\n'
        f'    "illustration_color": <color hex string>,\n'
        f'    "illustration_intensity": <float between 0 and 1>,\n'
        f'    "sticker_texture": <one of the textures below>,\n'
        f'    "sticker_color": <color hex string>,\n'
        f'    "sticker_intensity": <float between 0 and 1>,\n'
        f'    "sponsor_line_text": <string>,\n'
        f'    "sponsor_line_font_type": <one of the font types below>,\n'
        f'    "sponsor_line_font_size": <float between 0 and 1>,\n'
        f'    "sponsor_line_font_color": <color hex string>,\n'
        f'    "sponsor_line_position_x": <float between 0 and 1>,\n'
        f'    "sponsor_line_position_y": <float between 0 and 1>,\n'
        f'    "sponsor_line_angle": <int between 0 and 360>,\n'
        f'    "sponsor_name_text": <string>,\n'
        f'    "sponsor_name_font_type": <one of the font types below>,\n'
